...
sub updateIssue{
   ...
   my $fieldValue = $fields->[1];
   
   my $fieldmeta = getFieldsListMeta($JIRA_SERVER,$JIRA_USER,$JIRA_PASSWD);     
   my $fieldObject  = getFieldObjectFromName_FieldMeta($fieldmeta, $fields->[0]);
   my $updateJson = "";
    ...
    if( $fieldObject->{'schema'}->{'type'} eq 'string' or
       ...
       $fieldObject->{'schema'}->{'type'} eq 'version'   ) 
	{
	     $updateJson =  '{"update":{ "'.$fieldName.'": [ {"'.$operation.'":"'.$fieldValue.'" } ] } }';
    }
    if( $fieldObject->{'schema'}->{'type'} eq 'array' ){
	#TODO
        $updateJson =  '{"update":{ "'.$fieldName.'": [ {"'.$operation.'":"'.$fieldValue.'" } ] } }';
    ...
    if( $fieldObject->{'schema'}->{'type'} eq 'user' ){
	    $updateJson =  '{"update":{"'.$fieldObject->{id}.'":[{"'.$operation.'":{"name":"'.$fieldValue.'"}}]}}';
    }
   
   
   my $jiraServerRest = $JIRA_SERVER."/rest/api/2/issue/".$issueKey;
   print "[INFO] JSON:".$updateJson."\n";
   ...
   my $resp = $ua->request($req);   
   if ( !$resp->is_success ) {
		print  "[ERROR] Could not update $fieldName " . $resp->content . "\n";
		exit(0);
	}
